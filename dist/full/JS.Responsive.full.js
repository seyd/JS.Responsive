	 ...
	 * */
	  ...
	  */
		$C.isDaylight = function () {
			...
			if (!positionLat) throw new Error('Location not provided! You need to provide location, via setLocation( lan, lng ) method first!');
	
			if (daylightTimeout) clearTimeout(daylightTimeout);
			...
			if (daylight)
				...
				daylightTimeout = setTimeout($C.isDaylight, sunset - now + 10);else if (now < sunrise)
				//                                               time to sunrise + 10ms reserve
				daylightTimeout = setTimeout($C.isDaylight, sunrise - now + 10);else
